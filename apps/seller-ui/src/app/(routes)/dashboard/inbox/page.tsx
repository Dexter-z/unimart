        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative z-10 w-full max-w-2xl h-[80vh] bg-[#18181b] border border-[#232326] rounded-2xl flex flex-col">
                ...
                <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
                    ...
                    {messages.map((m, idx) => {
                        const fallbackKey = `${m.createdAt}-${m.senderId || 'unknown'}-${idx}`;
                        const key = m.id || fallbackKey;
                        return (
                            <div key={key} className={`flex ${m.senderType === 'seller' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`${m.senderType === 'seller' ? 'bg-[#ff8800] text-[#18181b]' : 'bg-[#232326] text-white'} px-3 py-2 rounded-2xl max-w-[75%]`}>
                                    {m.content}
